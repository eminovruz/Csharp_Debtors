...
class Program
{
    ...
    static void Main()
    {
        ...
        //debtors.Where(s => s.Email.EndsWith("rhyta.com") || s.Email.EndsWith("dayrep.com")).Show();

        // 3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        //debtors.Where(s => s.BirthDay.Year <= 1996 && s.BirthDay.Year > 1970).Show();

        // 4) Borcu 5000 - den cox olmayan borclularic cixartmag

        //debtors.Where(s => s.Debt < 5000).Show();

        // 5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

        //debtors.Where(s => s.FullName.Length > 18 && s.Phone.Count(d => d == '7') >= 2).Show();

        // 7) Qishda anadan olan borclulari cixardmaq

        //debtors.Where(s => s.BirthDay.Month == 1 || s.BirthDay.Month == 2 || s.BirthDay.Month == 3).Show();
        ...
        // var avg = debtors.Average(s => s.Debt);
        // debtors.Where(s => s.Debt > avg).Show();
        ...
        //         Console.WriteLine($"{debtor.FullName.Substring(debtor.FullName.LastIndexOf(' '))} {debtor.Debt} {debtor.Phone} {DateTime.Now.Year - debtor.BirthDay.Year} ");
        ...
        //debtors.Where(s => s.FullName.Contains("s") && s.FullName.Contains("m") && s.FullName.Contains("i") && s.FullName.Contains("l") && s.FullName.Contains("e")).Show();
